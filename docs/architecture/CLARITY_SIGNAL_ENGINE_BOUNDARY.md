...
flowchart LR
  ...
  O -. "schema v1: no model call" .-> S["Semantic abstention boundary"]
...
| Deterministic → semantic | Sanitised type only; exact inventory; offline flags; schema v1 prohibits model execution | Semantic abstains without importing or calling a model; deterministic path may continue |
